...
static int length_substring(const std::string &input) {
  int max_length = 0, left = -1; // left指向的是上一个之前出现的重复字符的索引，则最右边字符索引减去该索引就是不重复的长度
  ...
  for (int i = 0; i < length; ++i) {
    ...
    map_string[input[i]] = i; // 这里，如果有重复字符的话，会只记录最后一个索引，非常棒
...
static std::string longest_substring(const std::string &input){
  ...
  std::string ret_string = input.substr(idx_left+1, idx_right-idx_left); //idx_left是上一个重复字符，要排除掉
  printf("%s\n", ret_string.c_str());
  return ret_string;
}
