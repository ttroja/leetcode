...
empty. Return true  if s  is a valid string, otherwise, return false.
...
bool is_valid(std::string str) {
  for (auto pos = str.find("abc"); pos != std::string::npos;
       pos = str.find("abc")) {
    str.erase(pos, 3);
  }
  return str.empty();
...
int main() {
  ASSERT(is_valid("aabcbc") == true);
  ASSERT(is_valid("abcabcababcc") == true);
  ASSERT(is_valid("abccba") == false);
  ASSERT(is_valid("cababc") == false);
